 ...
 * Built by Aisha Tariq Khan
 *
 * ARCHITECTURE: Retrieval-Augmented Generation (RAG) Workflow
 * ─────────────────────────────────────────────────────────────
 * 1. PDF Ingestion   → pdf.js extracts raw text per page
 * 2. Chunking        → text split into overlapping page-chunks with metadata
 * 3. Retrieval       → keyword search finds top-K relevant chunks per query
 * 4. Augmentation    → retrieved chunks injected into Claude's context
 * 5. Generation      → Claude answers ONLY from provided context (no hallucination)
 * 6. Citation        → prompt enforces [§X.X, p.N] format for every claim
 */
...
async function loadPDF(file) {
  ...
  document.getElementById('uploadBtn').textContent = 'Parsing…';
  ...
  try {
    ...
    document.getElementById('docMeta').textContent = `${totalPages} pages · ${formatNum(wordCount)} words`;
    ...
    document.getElementById('statRisks').textContent = '—';
    ...
    addAIMessage(
      ...
      `📄 **${totalPages} pages** · **${formatNum(wordCount)} words** · **${state.chunks.length} retrieval chunks**\n\n` +
...
function buildRAGPrompt(query, chunks) {
  ...
  const systemPrompt = `You are ILM RAG, a precise and rigorous legal document analyst built for a law firm. You have been given extracted text from a legal document, organized by page number.

CRITICAL RAG RULES — you MUST follow ALL of these:

1. CITATION REQUIRED: Every factual statement MUST be followed by a citation in the format [p.N] where N is the page number. If you reference a section number visible in the text, use [§X.X, p.N].
...
4. STRUCTURE: Use clear formatting — bullet points for lists, bold for key terms.

5. LEGAL PRECISION: Flag potential risks, ambiguities, or unusual clauses with ⚠️.
...
function extractCitationsFromResponse(text) {
  const matches = text.match(/\[(?:§[\d.]+,\s*)?p\.?\s*\d+\]/g) || [];
...
async function runAnalysis() {
  ...
  document.getElementById('dashContent').innerHTML = '<div class="no-doc-state"><div style="font-size:32px">⏳</div><p>Claude is analyzing your document…</p></div>';
  ...
  try {
    ...
    document.getElementById('statRisks').textContent = analysis.risks?.length || '—';
    renderDashboard(analysis);
    state.analysisRun = true;
    btn.textContent = '✅ Analysis Complete';
    setStatus('idle', 'Ready');
  } catch (err) {
    document.getElementById('dashContent').innerHTML = `<div class="no-doc-state"><div style="font-size:32px">❌</div><p>Analysis failed: ${err.message}</p></div>`;
...
Return ONLY a valid JSON object with this exact structure — no preamble, no markdown backticks:
{
  "risks": [
    {
      "level": "high" | "medium" | "low",
      "description": "Concise risk description",
      "reference": "§X.X or Section name + page if visible"
    }
  ],
  "dates": [
    {
        ...
        ${analysis.risks.map(r => `
          <div class="risk-row">
            ...
            <div class="risk-body">
              ...
              ${r.reference ? `<div class="risk-ref">📌 ${escapeHtml(r.reference)}</div>` : ''}
            </div>
          </div>`).join('')}
      </div>`;
    c.appendChild(card);
...
function addAIMessage(text, cites, chunksUsed) {
  ...
  if (cites && cites.length) {
    html += `<div class="citations">${cites.map(c => `<span class="cite-pill">📌 ${c}</span>`).join('')}</div>`;
  }

  if (chunksUsed) {
    html += `<div class="msg-meta">${chunksUsed} document chunks retrieved · Citation-grounded response</div>`;
  }
...
function formatAIText(text) {
  return escapeHtml(text)
    ...
    .replace(/\[([^\]]+)\]/g, '<span class="cite-pill" style="font-size:11px;display:inline;">📌 [$1]</span>')
    .replace(/\n/g, '<br>');
}
